...
public class AudioSpecification implements Comparable<AudioSpecification>, Serializable {
    ...
    private AudioDecoder getDecoderFor(int enc) {
        try {
            return AudioDecoder.getAudioDecoderFor(enc);
        } catch (Exception e) {
            log.error(e, "AudioSpecification.getDecoderFor()");
            return null;
        }
    }
    ...
    public AudioDecoder getDecoder() {
        if (decoder == null) {
            if (isResultEncodingSet())
                decoder = getDecoderFor(resultEncoding); //decoder for known result
            else
                decoder = getDecoderFor(encoding); //decoder for expected result
        }
        return decoder; //will return null in case of exception(s) in getDecoderFor()
    }
