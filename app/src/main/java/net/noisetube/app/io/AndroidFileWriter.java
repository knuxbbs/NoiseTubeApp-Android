...
public class AndroidFileWriter extends FileWriter {
    ...
    public void open(int fileExistsStrategy, int fileDoesNotExistStrategy) throws Exception {
        ...
        if ((new File(fullPath)).exists()) {   //file already exists
    ...
    @Override
    public void rename(String newName, int fileExistsStrategy) throws Exception {    //TODO make sure the FileIO.FILE_DOES_NOT_EXIST_STRATEGY_CREATE is ok?
        if (writer != null)
            close();
